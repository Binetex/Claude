...
## ПРОДОЛЖЕНИЕ 2: production-готовность + защита инвариантов

По запросу «продолжай улучшать и подготавливать сайт». Все изменения проверены на локальной БД.

### Route-состояния (production-готовность) ✅
Ранее их не было вообще (навигация показывала пустоту, рантайм-ошибки падали в дефолтный оверлей).
Добавлено на базе дизайн-системы:
- `loading.tsx` для всех трёх групп дашборда (owner/florist/callcenter) — скелетон `CardListSkeleton`.
- `error.tsx` для всех трёх групп + общий клиентский `components/RouteError.tsx` (ErrorState + «Повторить»,
  без показа технических деталей пользователю).
- `app/not-found.tsx` — дружелюбная 404 (проверена визуально: 🌸 + «На главную»).
Коммит: `feat(ui): add route-level loading skeletons, error boundaries, and friendly 404`.

### Тесты ролевой видимости (security) ✅
`modules/orders/serialize.test.ts` — 12 тестов на чистых объектах (без БД), гварды самого
чувствительного инварианта: колл-центр без финансов/цен; флорист видит только свою цену
(и в MAKER_ONLY, и в FULL — никогда прибыль владельца/себестоимость доставки/цену клиента);
владелец видит всё. Ловит регрессию-утечку финансов. Коммит: `test(orders): guard role-based finance visibility`.

### SEO / mobile ✅
Внутренний инструмент помечен `robots: noindex, nofollow` (не индексировать поисковиками);
добавлен корректный `viewport` (device-width) + `theme-color` для мобильных. Проверено в браузере
(мета присутствует). Коммит: `feat(seo): noindex internal dashboard, add mobile viewport`.

### Проверки (итог продолжения 2)
typecheck ✅ · lint ✅ · build ✅ · **81 тест passed** (+ новые route/serializer тесты), 8 skipped;
единственный не-зелёный файл — `composition` (ограничение PGlite, не код). Визуально проверено:
404, отсутствие регрессий на Orders, консоль без ошибок, `noindex`-мета применена.

### Осталось как кандидаты (не блокеры)
- Полный визуальный редизайн Orders (текущий вид уже качественный; глубокий редизайн — по желанию владельца).
- Воркер для `jobs`/событий + персистентный outbox (см. PROPOSED_SCHEMA_CHANGES §2–3).
- Реальные интеграции/уведомления (нужны credentials).
- Подключение мониторинга ошибок в `RouteError` (сейчас только console.error).
