...
## 0. Обновление 2026-07-21 (текущее состояние)

Полный список изменений — в [`CHANGELOG.md`](../CHANGELOG.md). Кратко, что изменилось с базового
handoff (2026-07-17):

- **Интеграции доведены до боевых** (в базовом handoff они были заглушками):
  - **QUO (ex-OpenPhone)** — коммуникации SMS/звонки/записи, привязка к заказам, «Нераспознанные»,
    backfill. Live активирован для магазина **TheFlow** (`+13238008421`).
  - **WooCommerce** — реальный приём заказов, классификация оплаты, авто-назначение флориста,
    billing-адрес отправителя, Fees→чаевые, «Ошибка оплаты».
  - **Burq** — draft-first доставка, POD, retry, per-site dropoff-инструкции, времена доставки в UI.
- **Флористы в проде есть** (в базовом handoff было ноль): напр. Анита (основной), Наташа — для
  TheFlow. Авто-назначение работает для Shopify и WooCommerce.
- **Прибыль** теперь учитывает чаевые.
- **Данные (разово, 2026-07-21):** tip-backfill (67 заказов), recompute прибыли (52), удаление 2
  тест-заказов, откат backfill-инцидента (155 старых заказов). Детали и урок про `fullHistory` —
  в `CHANGELOG.md`.
...
- **`src/messaging/*`** (`MessagingService`, `subscribers.ts`, `templates.ts`,
  ...
  переписать обработчик на уже рабочий `modules/messaging/channels/*` (как сделано для
...
- UI создания пользователей («Настройки → Пользователи», `/dashboard/settings/users`) с выбором роли (основной/второстепенный флорист, колл-центр).
...
- В production **нет ни одного флориста/колл-центра** — только владелец. Нужно зайти в «Настройки → Пользователи» (`/dashboard/settings/users`) и создать хотя бы одного флориста, иначе новые заказы Shopify продолжат попадать в «Требует назначения».
...
1. **[Владелец, вручную]** Создать хотя бы одного флориста через «Настройки → Пользователи» (`/dashboard/settings/users`).
