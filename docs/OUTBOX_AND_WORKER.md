...
> **СТАТУС: развёрнуто на production (floremart.com).** Миграция `20260718040000_outbox_events`
> применена к prod-БД (Neon `neondb`); процесс `floremart-worker` запущен в PM2 из
> `ecosystem.worker.config.js`. Провайдеры сообщений — **mock** (реальной отправки нет);
> `eventBus.publish` в живых потоках пока НЕ заменён на `publishEvent` — outbox работает,
> но события в него ещё не публикуются из бизнес-логики (следующий шаг, за флагом).
...
| `WORKER_HEARTBEAT_FILE` | файл отметки живости | `logs/worker-heartbeat.json` |
| `WORKER_HEARTBEAT_MAX_AGE_MS` | возраст отметки, после которого воркер считается мёртвым | `120000` |
...
## Живость воркера (`/api/health/worker`)

Dead-letter-алерт в Telegram отправляет САМ воркер, поэтому мёртвый воркер ни на что не
жалуется — а вместе с ним молча встают SMS, email, создание доставок, fulfillment и пересчёт
финансов. Чтобы это перестало быть тихим отказом, цикл воркера ставит отметку живости, а
приложение отдаёт её наружу.

- Пишет: `src/outbox/heartbeat.ts` — из цикла `OutboxWorker.start()` **после каждого тика**
  (а не отдельным `setInterval`): отметка означает «оборот цикла реально прошёл», а зависший
  на внешнем вызове воркер жив для ОС, но очередь не двигает. Запись троттлится до одной раз
  в 30 с; ошибки записи глотаются — очередь важнее сигнала о ней.
- Файл, а не таблица: воркер и Next.js — два процесса на одном VPS с общим `cwd`, таблица
  потребовала бы миграции и записи в БД каждые полминуты ради значения, которое не нужно
  хранить дольше минуты. `logs/` есть на всех окружениях и деплоем не трогается.
- Читает: `GET /api/health/worker` → `200 {worker:"alive"}` либо
  `503 {worker:"stale"|"no_heartbeat"}` + `workerId`, `tickAt`, `ageMs`.

**Отдельно от `/api/health` сознательно.** Тот эндпоинт — гейт успешного деплоя в `deploy.sh`
и проверка веб-процесса: если бы он падал из-за воркера, мёртвый воркер откатывал бы деплои и
мог увести сайт в maintenance. Порог 120 с заведомо больше интервала записи и паузы на
`pm2 reload`, поэтому деплой ложную тревогу не даёт.

Чтобы это работало, внешний аптайм-монитор должен опрашивать **оба** URL: `/api/health`
(сайт) и `/api/health/worker` (фон). Сам по себе эндпоинт никого не уведомляет.
...
```
**Развёрнуто:** `floremart-worker` запущен через `pm2 start ecosystem.worker.config.js` +
`pm2 save`. Рабочий `ecosystem.config.js` НЕ менялся (worker — отдельным файлом). Важно:
имя файла должно оканчиваться на `.config.js`, иначе PM2 трактует его как скрипт, а не конфиг.
