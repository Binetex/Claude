...
Ниже описано состояние Stage 2/3; часть уже заменена дневной моделью, см. «Состояние
продакшена».
...
- **Финансовый модуль ещё НЕ открыт работникам** — только владельцу для тестов. Заказы при
  ...
  Начисления Olga и Наташе обнуляются последним действием перед открытием доступа.
- **Финансы перестраиваются на дневную модель** (не закончено, пять коммитов не
  задеплоены). Суть: `DayFinance` — одна ИЗМЕНЯЕМАЯ строка на (профиль, день) вместо
  позаказных снимков с ревизиями; дневная закупка цветов вычитается один раз на уровне
  дня, а не раскладывается по заказам; долг флориста ВЫВОДИТСЯ из этих строк и нигде не
  хранится, поэтому сторно и корректировок начислений больше нет. Записываются только
  решения владельца — выплаты, бонусы, ручные корректировки. Равенство плоской формулы
  прежней доказано интеграционным тестом до цента. Что осталось: удалить `accrual.ts`,
  начисляющую часть `primaryShare.ts`, `snapshot.ts`, распределение в `calc.ts`,
  `intervals.ts` и периоды действия у настроек; перевести детектор на `computeDay`.
  Порядок важен: сначала удалить код — компилятор сам покажет, какие тесты проверяли
  удалённое (их выбросить), остальные перевести на `floristBalance`.
