...
/**
 * Девять пунктов, порядок — по частоте работы: сверху то, куда заходят каждый день.
 *
...
function ownerNav(): NavItem[] {
  return [
    ...
    { href: "/dashboard/automations", label: "Автоматизации" },
    {
      ...
      label: "Настройки",
      // Burq и «Нераспознанные» лежат вне группы (owner) — иначе на них навесится
      // requireRole("OWNER") и разделы закроются для остальных ролей, ради которых их и
      // вынесли. Поэтому пункт подсвечивается и на их адресах тоже.
      match: ["/dashboard/settings", "/dashboard/burq", "/dashboard/communications"],
    },
  ];
}
