 ...
 *  1) обесцениваем неподтверждённые заказы (отменён/возврат/неоплачен), если есть другие;
...
/**
 * Заказы, которые НЕ считаются подтверждёнными бизнесом и уступают живым при неоднозначности.
 *
 * `AWAITING_PAYMENT` здесь потому, что клиенты регулярно отправляют заказ дважды за считанные
 * секунды: оплачивается вторая попытка, а первая навсегда остаётся неоплаченным дублем с ТЕМ ЖЕ
 * телефоном и ТОЙ ЖЕ датой доставки (THEFLOW-20395 и -20396, 8 августа 2026, 53 секунды между
 * ними). Для матчера два таких заказа были одинаково близки → `ambiguous` → ответ клиента не
 * привязывался ни к одному. При этом ИСХОДЯЩИЕ Floremart проставляет orderId сразу при отправке,
 * минуя матчер, — поэтому со стороны выглядело как «наши сообщения уходят, а ответы не приходят».
 *
 * Обесценивание, а не отсев: если ВСЕ кандидаты неоплаченные (или он такой один), заказ всё равно
 * матчится — переписка по неоплаченному заказу тоже нужна.
 */
const NON_ACTIVE_STATUSES = new Set(["CANCELLED", "REFUNDED", "AWAITING_PAYMENT"]);
