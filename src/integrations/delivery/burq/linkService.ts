...
/**
 * Статусы, из которых текущую попытку можно заменить БЕЗ вопроса.
 *
 * Это терминально-провальные (отменена, провал, проблема, возврат) И НЕОТПРАВЛЕННЫЕ ЧЕРНОВИКИ.
 * Черновик — это наша заготовка, которую в Burq никто не оформлял: ни курьера, ни денег, ни
 * трека. Спрашивать «у заказа уже есть активная доставка, заменить?» про такую заготовку значит
 * задавать вопрос на пустом месте — а человек в этот момент как раз привязывает НАСТОЯЩУЮ
 * доставку, которую флорист завёл в кабинете Burq руками (OHARA-1073, 21.09.2026: там наш
 * черновик и доставка флориста были созданы в одну минуту).
 *
 * Подтверждение остаётся там, где оно и нужно: доставка ЖИВАЯ — курьер назначен, едет или уже
 * забрал букет. Подменить её молча значит спрятать реального курьера, за которого уже платят.
 */
