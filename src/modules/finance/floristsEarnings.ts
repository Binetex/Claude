...
/**
 * Число доставленных заказов флориста — колонка «Доставлено» в списке флористов.
 *
 * Переехала из отдельного review.ts: после сноса «Очереди разбора» там оставалась одна эта
 * функция, и файл читался как самостоятельный модуль, которым давно не был.
 *
 * Гейтится датой старта начислений: без неё в счёт попали бы 89 доставленных заказов
