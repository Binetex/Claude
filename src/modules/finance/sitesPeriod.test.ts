...
import { resolveSitesPeriod } from "./sitesPeriod";
...
describe("период страницы «Магазины»", () => {
  it("по умолчанию — текущий месяц", () => {
    const p = resolveSitesPeriod({}, NOW);
  ...
  it("«сегодня» — по таймзоне магазина, а не по UTC", () => {
    ...
    const p = resolveSitesPeriod({ period: "today" }, NOW);
  ...
  it("«вчера» — один день, предыдущий", () => {
    const p = resolveSitesPeriod({ period: "yesterday" }, NOW);
    expect(iso(p.from)).toBe("2026-08-13");
    expect(iso(p.to)).toBe("2026-08-13");
  });

  it("«неделя» — последние 7 дней, включая сегодня", () => {
    const p = resolveSitesPeriod({ period: "week" }, NOW);
    expect(iso(p.from)).toBe("2026-08-08");
    expect(iso(p.to)).toBe("2026-08-14");
  });

  it("месяц заканчивается последним числом, а не 30-м всегда", () => {
    expect(iso(resolveSitesPeriod({}, new Date("2026-02-10T12:00:00.000Z")).to)).toBe("2026-02-28");
    expect(iso(resolveSitesPeriod({}, new Date("2028-02-10T12:00:00.000Z")).to)).toBe("2028-02-29");
    expect(iso(resolveSitesPeriod({}, new Date("2026-04-10T12:00:00.000Z")).to)).toBe("2026-04-30");
  });

  it("свой диапазон берётся как есть", () => {
    const p = resolveSitesPeriod({ period: "range", from: "2026-07-01", to: "2026-07-10" }, NOW);
  ...
  it("перевёрнутый диапазон разворачивается, а не даёт пустоту", () => {
    const p = resolveSitesPeriod({ period: "range", from: "2026-07-10", to: "2026-07-01" }, NOW);
    expect(iso(p.from)).toBe("2026-07-01");
    expect(iso(p.to)).toBe("2026-07-10");
  });

  it("одна дата с обеих сторон подписывается как день, а не как диапазон", () => {
    const p = resolveSitesPeriod({ period: "range", from: "2026-07-04", to: "2026-07-04" }, NOW);
    expect(p.label).toBe("4 июля 2026");
  });

  it("половина диапазона достраивается сегодняшним днём", () => {
    const p = resolveSitesPeriod({ period: "range", from: "2026-08-01" }, NOW);
    expect(iso(p.from)).toBe("2026-08-01");
    expect(iso(p.to)).toBe("2026-08-14");
  });

  it("мусор в параметрах не роняет страницу — остаётся месяц по умолчанию", () => {
    expect(resolveSitesPeriod({ period: "квартал" }, NOW).kind).toBe("month");
    const broken = resolveSitesPeriod({ period: "range", from: "2026-13-45", to: "нет" }, NOW);
