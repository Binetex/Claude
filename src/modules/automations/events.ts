...
export type AutomationTriggerPayload = {
  ...
  /**
   * Запустить ТОЛЬКО это правило, а не все правила события. Так работает шаг цепочки «не
   * ответили»: следующее правило выбрано владельцем поимённо, и подтягивать соседей по типу
   * события нельзя. Пусто — обычное событие заказа: срабатывают все подходящие правила.
   */
  automationId?: string;
};
