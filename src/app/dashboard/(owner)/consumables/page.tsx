        ...
        <CardBody>
          <form method="get" className="flex flex-wrap items-end gap-2 text-xs">
            <label className="flex flex-col gap-0.5">
              Месяц
              <input type="month" name="month" defaultValue={month} className="rounded border border-slate-300 px-2 py-1" />
            </label>
            <label className="flex flex-col gap-0.5">
              Флорист
              <select name="florist" defaultValue={floristId ?? ""} className="rounded border border-slate-300 px-2 py-1">
            ...
            <button type="submit" className="rounded bg-sky-600 px-3 py-1 font-medium text-white">Показать</button>
            <a
              href={`/dashboard/consumables/export?month=${month}${floristId ? `&florist=${floristId}` : ""}`}
