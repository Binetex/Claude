...
import { listSmsTriggers } from "@/modules/sms/triggers";
import { SMS_VARIABLES } from "@/modules/sms/variables";
import { audienceLabel, jobStatusLabel, jobStatusClass, maskPhoneDisplay } from "@/modules/sms/display";
...
export default async function EditAutomationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sites, recentOrders, jobs] = await Promise.all([
    ...
    prisma.smsAutomationJob.findMany({
