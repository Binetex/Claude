...
import { OrderQueueList } from "./OrderQueueList";
...
export function OrdersTable({
  ...
  const card = (o: OrdersTableOrder) => ({
    desktop: <DesktopCard o={o} ind={commIndicators[o.id]} hideFinance={hideFinance} hideFlorist={hideFlorist} hrefBase={hrefBase} backQuery={backQuery} sideAmountLabel={sideAmountLabel} />,
    mobile: <MobileCard o={o} ind={commIndicators[o.id]} hideFinance={hideFinance} hideFlorist={hideFlorist} hrefBase={hrefBase} backQuery={backQuery} />,
  });

  // Очередь дня рисуется отдельным клиентским списком: плашки он получает уже собранными на
  // сервере и только держит порядок, чтобы перестановка была мгновенной. Разбивки по дням
  // здесь нет и быть не может — очередь существует внутри одного дня.
  // Пустой день уходит в общий путь ниже: там есть «Заказов не найдено», а пустая очередь
  // нарисовала бы просто ничего.
  if (queueMode && orders.length > 0) {
    return (
      <OrderQueueList
        editable={queueMode === "edit"}
        items={orders.map((o) => ({ id: o.id, ...card(o) }))}
      />
    );
  }
  ...
  for (const o of orders) {
    ...
    const c = card(o);
    desktopItems.push(<div key={o.id}>{c.desktop}</div>);
    mobileItems.push(<div key={o.id}>{c.mobile}</div>);
  }
