...
describe("#4 — объединённый блок сохраняет нужное содержимое", () => {
  ...
  /**
   * 21.09.2026: кнопка пересоздания была только у ОТМЕНЁННОЙ попытки, а чаще нужна у живой —
   * Burq отменил доставку у себя, курьер уехал не забрав букет. Её видит и флорист: панель
   * одна на три кабинета.
   */
  /** Ответ сервера у формы привязки виден ОДНОЙ плашкой сверху — включая вопрос про замену. */
  it("вопрос про замену живой доставки показывается плашкой, а не только кнопкой", () => {
    const form = readFileSync(join(dir, "BurqLinkForm.tsx"), "utf8");
    expect(form).toContain("needsConfirm && (");
    expect(form).toContain("Подтвердите замену кнопкой ниже");
    expect(form).toContain("Заменить живую доставку");
  });
  it("пересоздание доступно у живой доставки и предупреждает про двух курьеров", () => {
    expect(panel).toContain("Пересоздать доставку");
    expect(panel).toContain("приедут два курьера");
    expect(panel).toContain("needsConfirm");
  });
});
