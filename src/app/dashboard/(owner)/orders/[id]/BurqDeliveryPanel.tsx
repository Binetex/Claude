...
function ActualCostBlock({
  ...
  // Флористу и колл-центру — только ПОДТВЕРЖДЁННАЯ человеком стоимость. Цифра от Burq владельцу
  // показывается как известная (расчёт дня её принимает), но остальным она ничего не объясняет:
  // это не их деньги, а вопросов про финансы добавляет.
  if (!canEdit && actualCost.confirmedAt === null) return null;
